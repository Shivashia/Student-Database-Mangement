...
label=Label(text="PATEL",font=("times new roman",35),bg="Yellow")
label.pack(side=TOP ,fill=X)
user1=Label(text="USERNAME",font=("arial",23))
user1.place(x=610,y=120)
user=Entry(width=17,bd=5,font=("arial",20))  #login entries
...
#-------------Student details display-------------------------------------------------#        
    def studentid():
        ...
        mainlabel = Label(rot, text="ENQUIRY DETAILS", font=("times new roman", 35), bg="Yellow")
        ...
        ttk.Style().configure("Treeview.Heading", background="blue", foreground="palevioletRed") #student enquiry display
        ...
        with conn:
            ...
            for row1 in cur.fetchall():
                chat1.insert('', 0, text=row1[0], values=(row1[1] ,row1[2],row1[3]))   # fetching and displaying the data from the table kistar
...
#------------------------------------------Visiting enquiry------------------------------------------------------------#
    def viewenquiry2():
        ...
        mainlabel =Label(rt, text="VISITOR", font=("times new roman", 35), bg="Yellow")
        mainlabel.pack(side=TOP, fill=X)
    ...
    def distroy5():
        ...
        window()
     #------------------------------Main WIndow--------------------------------------------#   
    mainlabel= Label(secondw,text="PATEL", font=("times new roman", 35), bg="Yellow")
    mainlabel.pack(side=TOP, fill=X)
...
with conn:
    ...
    cur.execute('CREATE TABLE IF NOT EXISTS FEEINSTALLMENT (id int ,TOTAL FEE INT, REMAIN FEE INT, PAID FEE INT ,INSTALLMENT INT,DATE)')
...
def enquiry1():
    ...
    global entry24
    global box2
#---------------Back button Function for enquiry window-----------------------------#    
    def enquiry1destroy():
        enquiry1.destroy()
        second()
    ...
    box2['values'] = ['TO LEARN PROGRAMMING', 'TO LEARN MACHINE LEARNING', 'FEE DETAILS']
    box2.current(0)
...
def cat():
    ...
    with conn:
        ...
        for row24 in cur.fetchall():
          ...
          cur.execute(' SELECT SUM(INSTALLMENT) FROM FEEINSTALLMENT WHERE id=? GROUP BY id ', (FE,)) #query used for adding and displaying total fee
...
def fee_add():
    ...
    entry28.configure(state="normal")
    entry26.configure(state="normal")
    cur.execute('INSERT INTO FEEINSTALLMENT(id , TOTAL,INSTALLMENT, DATE)VALUES(?,?,?,?)', (FE, x,y, today,))
...
#----------------------search function for stud details--------------------------------------------#	        
def golu():
     entry1.delete(0,END)
     entry2.delete(0,END)
     entry3.delete(0,END)
...
def delete():
    ...
    if box>0:  #will search when there are entries hence >0
        SEARCH=search.get()
        NAME=name.get()
        SUR=sur.get()
        EMAIL=email.get()
